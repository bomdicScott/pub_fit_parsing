...
def print_record(rec, ):
    global record_number, time,distance,heartrate,velocity_smooth ,lat,long,altitude,watts,cadence,temp,grade_smooth,file_path
    ...
    for field in rec.fields:
        ...
        if(header == True):
            if(field.name == "timestamp"):
                ...
                header_dict = {"time":field.data}
            else:
                header_dict[field.name] = field.data
        if field.name == 'timestamp':
...
if not filenames:
    ...
    for fname in glob.glob('C:\\Users\\sean\\Documents\\python-fitparse-master\\tests\\data\\*.fit'):
        ...
        if time != []:
            ...
            with open(file_path+"/Fit_header.json","w") as Fit_json:
                json.dump(header_csv,Fit_json)
                Fit_json.close()
